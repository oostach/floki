...
const TodoList = ({ items, deleteItem, toggleItem, enableEditMode }) => {
  return (
    <div className='todos-list mt-4'>
      <div className='todos-list-name mb-2 font-semibold text-lg text-sky-900'>My List</div>
      {
        items.sort((a, b) => b.id - a.id).map(item => {
          return <TodoItem key={item.id} item={item} deleteItem={deleteItem} toggleItem={toggleItem} enableEditMode={enableEditMode} />
        })
      }
    </div>
  )
